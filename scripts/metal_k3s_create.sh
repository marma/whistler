...
# pageSize, 2Mi) — that is what keeps VFIO's DMA pinning of a large GPU guest
...
hp_reserved="$(cat "$HP_2M" 2>/dev/null || echo 0)"
hp_1g="$(cat "$HP_1G" 2>/dev/null || echo 0)"
if (( hp_reserved > 0 )); then
  echo "    2Mi hugepages: ${hp_reserved} reserved ($(( hp_reserved / 512 )) GiB of guest RAM)"
else
  echo "    2Mi hugepages: NONE reserved — VMs will pend on 'Insufficient hugepages-2Mi'"
  echo "           at boot (survives reboots, no fragmentation; count is in 2MiB"
  echo "           units — 16384 is 32GiB):"
  echo "             kernel args  hugepagesz=2M hugepages=<count>"
  echo "           now (2MiB pages usually still allocate on a running host;"
  echo "           kubelet only reports the new capacity after a restart):"
  echo "             echo <count> | sudo tee ${HP_2M} && sudo systemctl restart k3s"
  echo "           or run guests on 4KiB pages: --set whistler.hugePages.pageSize=\"\""
fi
if (( hp_1g > 0 )); then
  echo "    1Gi hugepages: ${hp_1g} reserved — set whistler.hugePages.pageSize=1Gi"
  echo "           (or a template's resources.hugePageSize) to use them"
fi
