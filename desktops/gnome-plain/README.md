...
[`../streamer-selkies2`](../streamer-selkies2/) sidecar (`streamer: sidecar`),
which shares `/tmp/.X11-unix` / `/tmp/pulse` and injects `DISPLAY` /
`PULSE_SERVER`. Sidecar-mode counterpart of
[`../gnome-selkies2`](../gnome-selkies2/), as `xfce-plain` is to
`xfce-selkies2`.
...
1. **X11, not Wayland — by architecture.** Under Wayland, GNOME Shell *is* the
   ...
   without systemd-PID1 — full rationale in `../gnome-selkies2/Dockerfile`).
   Wayland-native GNOME needs a non-X capture point (later stage of the
   guest-unaware-display plan).
...
Identity is runtime-configurable like gnome-selkies2
