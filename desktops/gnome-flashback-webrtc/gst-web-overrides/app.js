...
var app = new Vue({
    ...
    data() {
        return {
            ...
            resizeRemote: true,
            scaleLocal: false,
            // True pixel-doubling: request half the physical resolution from
            // the server (normal-sized UI, no HiDPI upscaling of the desktop
            // itself), then stretch that smaller stream to fill the window
            // with nearest-neighbor scaling for crisp 2x2 blocks instead of
            // native full-resolution rendering.
            pixelDouble: false,
            debug: false,
            turnSwitch: false,
    ...
    methods: {
        ...
        // The resolution to request from the server: physical pixels (DPR-
        // multiplied) normally, or half that (logical/CSS pixels) when
        // pixelDouble is on, so the desktop renders at "normal" size instead
        // of native-HiDPI-sharp before being stretched back up client-side.
        getEffectiveResolution() {
            ...
            return webrtc.input.getWindowResolution();
        }
    },
    ...
    watch: {
        ...
        resizeRemote(newValue, oldValue) {
            ...
            app.windowResolution = app.getEffectiveResolution();
        ...
        pixelDouble(newValue, oldValue) {
            if (newValue === null) return;
            console.log("pixelDouble changed from " + oldValue + " to " + newValue);
            if (oldValue !== null && newValue !== oldValue) {
                webrtc.element.setAttribute("class", newValue ? "video scale pixelated" : "video scale");
                // Pixel-doubling needs remote resize on to actually shrink the
                // stream; turning it on implies resizeRemote (re-entrant watch
                // is a no-op if already true).
                if (newValue === true) app.resizeRemote = true;
                app.windowResolution = app.getEffectiveResolution();
                var res = app.windowResolution[0] + "x" + app.windowResolution[1];
                webrtc.sendDataChannelMessage('_arg_resize,true,' + res);
                // Reset DPI back to 96 (unscaled) so UI on the now-smaller
                // canvas renders at its normal, comfortable size rather than
                // being enlarged again on top of the pixel-doubled upscale.
                webrtc.sendDataChannelMessage('s,' + (newValue ? '1.0' : window.devicePixelRatio));
            }
            this.setBoolParam("pixelDouble", newValue);
        },
...
webrtc.input.onresizeend = () => {
    ...
    webrtc.sendDataChannelMessage("s," + (app.pixelDouble ? '1.0' : window.devicePixelRatio));
}
...
webrtc.onsystemaction = (action) => {
    ...
    if (action === 'reload') {
        ...
        if (app.resizeRemote === true) {
            ...
            // Normally remote_res is physical pixels, so dividing by DPR maps
            // it back to the CSS box size (1:1 native mapping). In pixelDouble
            // mode remote_res is already the logical/CSS size (half of
            // physical), so the CSS box is set directly to it — the browser
            // then stretches that smaller buffer up to fill the box, which
            // .pixelated (see CSS) renders as crisp 2x2 blocks instead of blur.
            var divisor = app.pixelDouble ? 1 : window.devicePixelRatio;
            webrtc.element.style.width = toks[0]/divisor+'px';
            webrtc.element.style.height = toks[1]/divisor+'px';
