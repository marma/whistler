...
# user. Started by the entrypoint; see there for the surrounding X/pulse setup.
#
...
# the entrypoint's already-running PulseAudio. gnome-session sends the whole
...
# gnome-shell + a curated daemon set directly sidesteps that entirely and, more
# importantly, keeps this image UNPRIVILEGED — the whole reason design/
# creating_desktops.md §8 picks GNOME 46/24.04 over the systemd-PID1 + privileged
# route that ../gnome-flashback-webrtc and ../gnome-grd had to take. The cost is
# only the logind-tied features (lock/suspend/seat switching), which are
# meaningless for a single-user streamed desktop.
set -e
